...
class FullContract extends Component {
  ...
  deleteProperty = (e, data) => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        axios
          .delete("http://localhost:5000/api/fullContract/" + data.key)
          .then(res => {
            console.log("res: ", res);
  ...
  render() {
    ...
    this.state.dataState.forEach(element => {
      ...
      data.push({
        key: `${element.id}`,
        FullContractCode: `${element.FullContractCode}`,
        PropertyCode: `${Object.values(element)[16]}`,
    ...
    const columns = [
      {
        title: "Mã hợp đồng",
        dataIndex: "FullContractCode",
        key: "FullContractCode",
        width: "15%"
      },
      {
...
export default connect(null, mapDispatchToProps)(FullContract);
